...
import com.test32.common.wrapper.google.common.primitives.Longs;
    ...
    while (true) {
      switch (k) {
        ...
        default:
          ...
          k >>= 1;
          if (k > 0) {
            MathPreconditions.checkNoOverflow(
    ...
    LARGE {
      ...
      private long times2ToThe32Mod(long a, long m) {
        ...
        do {
          ...
          a = com.test32.common.wrapper.google.common.primitives.UnsignedLongs.remainder(a << shift, m);
      ...
      long mulMod(long a, long b, long m) {
        ...
        if (result < 0) {
          result = com.test32.common.wrapper.google.common.primitives.UnsignedLongs.remainder(result, m);
        }
        // result < 2^63 again
        result += aLo * bHi; // aLo * bHi < 2^63, result < 2^64
        result = times2ToThe32Mod(result, m); // result < m < 2^63
        return plusMod(result, com.test32.common.wrapper.google.common.primitives.UnsignedLongs.remainder(aLo * bLo /* < 2^64 */, m), m);
      }
      ...
      long squareMod(long a, long m) {
        ...
        long hiLo = aHi * aLo * 2;
        if (hiLo < 0) {
          hiLo = com.test32.common.wrapper.google.common.primitives.UnsignedLongs.remainder(hiLo, m);
        }
        // hiLo < 2^63
        result += hiLo; // result < 2^64
        result = times2ToThe32Mod(result, m); // result < m < 2^63
        return plusMod(result, com.test32.common.wrapper.google.common.primitives.UnsignedLongs.remainder(aLo * aLo /* < 2^64 */, m), m);
      }
    };
